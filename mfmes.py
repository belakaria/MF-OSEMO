...
class MultiFidelityMaxvalueEntroySearch_NI:
    ...
    def Sampling_RFM(self):
        ...
        X_train_features, X_test_features = self.MF_rbf_fit_transform(
            feature_size=rbf_feature_size
        )

        A_inv = np.linalg.inv(
            (X_train_features.T).dot(X_train_features)
            + np.eye(rbf_feature_size * self.M) / self.RegModel.beta
        )
        ...
        standard_normal_rvs = np.random.normal(
            0, 1, (np.size(weights_mean), self.sampling_num)
        )
        weights_sample = np.matlib.repmat(
            np.c_[weights_mean], 1, self.sampling_num
        ) + L.dot(standard_normal_rvs)

        func_sample = (
            X_test_features.dot(weights_sample) * self.RegModel.y_std
            + self.RegModel.y_mean
        )
    ...
    def low_TNEntropy(self, max, high_mean, high_std, low_mean, low_std, cov):
        ...
        DQ_right = norm.ppf(1.0 - 1e-12, loc=low_mean, scale=low_std)
        inputs = (
            np.reshape(inputs, (1, 1, self.EntropyApproxNum)) * (DQ_right - DQ_left)
            + DQ_left
        )
        truncated_constant = norm.cdf((max - high_mean) / high_std)
        truncated_constant[truncated_constant <= 0] = 1e-15
        conditionedCDF = norm.cdf(
            (max - (high_mean + cov / np.power(low_std, 2) * (inputs - low_mean)))
            / np.sqrt(np.power(high_std, 2) - np.power(cov, 2) / np.power(low_std, 2))
        )
        ...
        TNEntropy = (
            ...
            / truncated_constant
            * pdf
            * (np.log(conditionedCDF) + np.log(pdf) - np.log(truncated_constant))
            * (DQ_right - DQ_left)
        )
        ...
        if (
            np.sum(np.isnan(truncated_constant)) > 0
            or np.sum(truncated_constant <= 0) > 0
        ):
    ...
    def calc_acq(self, max_sample):
        max_sample[max_sample < self.y_max + 5 * np.sqrt(self.sigma_epsilon)] = (
            self.y_max + 5 * np.sqrt(self.sigma_epsilon)
        )
        self.max_samples = max_sample
        normalized_max = (
            max_sample - np.c_[self.mean[(self.M - 1) * self.size :]]
        ) / np.c_[self.std[(self.M - 1) * self.size :]]
        ...
        high_mean = np.matlib.repmat(
            self.mean[(self.M - 1) * self.size :], self.M - 1, 1
        )
        high_std = np.matlib.repmat(self.std[(self.M - 1) * self.size :], self.M - 1, 1)

        low_TN_entropy = self.low_TNEntropy(
            np.atleast_3d(max_sample),
            np.atleast_3d(high_mean),
            np.atleast_3d(high_std),
            np.atleast_3d(self.mean[: (self.M - 1) * self.size]),
            np.atleast_3d(self.std[: (self.M - 1) * self.size]),
            np.atleast_3d(self.cov),
        )
        low_entropy = np.ravel(
            np.log(np.sqrt(2 * np.pi * np.e) * self.std[: (self.M - 1) * self.size])
        )
        ...
        for m in range(0, self.M - 1):
            acq_func[self.size * m : self.size * (m + 1)] = acq_func[
                self.size * m : self.size * (m + 1)
            ]
...
class MultiFidelityMaxvalueEntroySearch_TG:
    def __init__(
        self, mean, std, y_max, index, M, cost, size, RegressionModel, sampling_num=10
    ):
        self.mean = np.c_[mean]
        self.std = np.c_[std]
        self.y_max = y_max
    ...
    def calc_acq(self, max_sample):
        ...
        c = np.zeros(self.M)
        max_sample[max_sample < self.y_max + 5 * np.sqrt(1.0 / self.RegModel.beta)] = (
            self.y_max + 5 * np.sqrt(1.0 / self.RegModel.beta)
        )
