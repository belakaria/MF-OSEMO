...
from test_functions import get_benchmark_functions, get_cost
...
fidelity_iter = [
    np.array([intial_number for j in range(M[i] - 1)] + [1]) for i in range(len(M))
]
total_cost = sum(
    [
        sum(
...
for i in range(len(functions)):
    ...
    for m in range(M[i]):
        temp0 = temp0 + list(
            np.random.randint(size * m, size * (m + 1), fidelity_iter[i][m])
        )
...
for j in range(total_iterations):
    if j % 5 != 0:
        ...
        for i in range(len(functions)):
            GPs[i].optimized_fit(
                candidate_x[i][GP_index[i].tolist()], y[i][GP_index[i].tolist()]
            )
    ...
    for i in range(len(functions)):
        if fidelity_iter[i][M[i] - 1] > 0:
            ...
            y_max[i] = GP_mean[i][(M[i] - 1) * size :][
                np.argmax(
                    GP_mean[i][(M[i] - 1) * size :] + GP_std[i][(M[i] - 1) * size :]
                )
            ]
    # Acquisition function calculation
    for i in range(len(functions)):
        if approx == "NI":
            MFMES[i] = MFBO.MultiFidelityMaxvalueEntroySearch_NI(
                GP_mean[i],
                GP_std[i],
                y_max[i],
                GP_index[i],
            ...
            MFMES[i] = MFBO.MultiFidelityMaxvalueEntroySearch_TG(
                GP_mean[i],
                GP_std[i],
                y_max[i],
                GP_index[i],
    ...
    for i in range(sample_number):
        front = [
            [-1 * func_samples[k][l][i] for k in range(len(functions))]
            for l in range(size)
        ]
    ...
    for k in range(size):
        ...
        values = [
            float(sum(AF)) / i
            for AF, i in zip(list(itertools.product(*temp)), values_costs)
        ]
    ...
    cost_input_output.write(
        str(total_cost)
        + " "
        + str(candidate_x[i][new_index])
        + " "
        + str(np.array([y[i][new_index] for i in range(len(functions))]))
        + "\n"
    )
